<script lang="ts">
  ...
  let vocabularies = $derived(
    data.vocabularies.map((v: Vocabulary, i: number) => ({ id: i, ...v }))
  );
  let chosenVocabulary = $state.raw({ id: 0, ...data.vocabularies[0] });
  let stagingAreaVocabulary = $derived(chosenVocabulary || vocabularies[0]);
  let remainingVocabularies = $derived(
    vocabularies.filter((v: IndexedVocabulary) => v.id !== chosenVocabulary.id)
  );

  $effect(() => {
    chosenVocabulary = vocabularies[0];
  });
</script>
